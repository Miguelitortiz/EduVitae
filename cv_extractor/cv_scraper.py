    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            for r in page_rows(page):
                ...
                if full and not SKIP_RE.match(full):
                    rows.append(r)
    return rows
    ...
    while i < len(rows):
        ...
        if lft == "Área":
            if i+1 < len(rows) and not _is_label(rows[i+1]["left"]):
                ...
                if rgt_val.lower().endswith("en trámite") or rgt_val.lower().endswith("en tramite"):
                    d["discipline"] = rgt_val[:-10].strip()
                    d["professionalId"] = "En trámite"
                elif rgt_val.lower().endswith("trámite") or rgt_val.lower().endswith("tramite"):
                    d["discipline"] = rgt_val[:-7].strip()
                    d["professionalId"] = "En trámite"
                else:
...
def _split_and_parse_degrees(rows: list[dict]) -> list[dict]:
    """Split degree rows at any degree level, 'Nivel de estudios', or separator."""
    DEGREE_STARTS = set(LEVEL_MAP.keys()) | {"nivel de estudios"}
    blocks, cur = [], []
    for r in rows:
        lft_lower = r["left"].strip().lower()
        full = (r["left"] + " " + r["right"]).strip()
        is_sep = re.match(r"^\.{10,}$", full)
        is_start = lft_lower in DEGREE_STARTS

        if is_sep:
            if cur: blocks.append(cur); cur = []
        elif is_start and cur:
...
def _split_and_parse_certs(rows: list[dict]) -> list[dict]:
    """Split cert rows at certificate category headers (e.g. Diplomado, Curso, Taller, etc.)."""
    blocks, cur = [], []
    for r in rows:
        lft = r["left"].strip()
        full = (lft + " " + r["right"]).strip()
        is_sep = re.match(r"^\.{10,}$", full)

        if is_sep:
            if cur: blocks.append(cur); cur = []
        elif lft.lower() in CERT_HEADERS:
...
def _split_and_parse_courses(rows: list[dict]) -> list[dict]:
    """Split course rows at 'Curso impartido DES' header pattern."""
    blocks, cur = [], []
    for r in rows:
        lft = r["left"].strip()
        full = (lft + " " + r["right"]).strip()
...
def _split_and_parse_theses(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        ...
        full = (lft + " " + r["right"]).strip()
        is_sep = re.match(r"^\.{10,}$", full)
        is_start = "Nombre del estudiante" in lft
...
if __name__ == "__main__":
    import sys
    path = sys.argv[1] if len(sys.argv) > 1 else "/Users/miguel/Documents/Projects/EduVitae/CV2026_WalterMata.pdf"
    result = scrape(path)

    out = "cv_extracted.json"
    ...
    for t in hr["theses"]:
        print(f"  {t.get('studentName','?')[:40]}: {t.get('thesisTitle','?')[:55]}")
